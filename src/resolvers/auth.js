...
export const authResolvers = {
  ...
  Mutation: {
    register: async (_, { input }, { user: currentUser, models }) => {
      try {
        ...
        const existingUser = await models.User.findByEmail(email) || await models.User.findByUsername(username);
        if (existingUser) {
    ...
    login: async (_, { input }, { models }) => {
      try {
        ...
        // Verify password
        const isValid = await models.User.comparePassword(password, user.password);
        if (!isValid) {
